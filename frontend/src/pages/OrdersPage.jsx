    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      {orders.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-sm text-zinc-400 mt-1 mb-6">Your order history will appear here</p>
          <Link to="/products" className="bg-zinc-900 text-white px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-zinc-700 transition-colors">
            Start Shopping
          </Link>
        </div>
      ) : (
          ...
          {orders.map((order) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  <p className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">Order</p>
                  <p className="font-mono text-sm font-bold text-zinc-800 mt-0.5">#{order._id.slice(-8).toUpperCase()}</p>
                </div>
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-xl border ${STATUS_STYLES[order.orderStatus] || 'bg-zinc-100 text-zinc-600 border-zinc-200'}`}>
                  {order.orderStatus}
                </span>
              </div>
              <div className="flex gap-2 mb-4">
                {order.items.slice(0, 4).map((item, i) => (
                  <img key={i} src={item.image || 'https://placehold.co/60x60'} alt={item.name} className="w-11 h-11 object-contain mix-blend-multiply rounded-xl shrink-0" />
                ))}
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs text-zinc-400">{new Date(order.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                <div className="flex items-center gap-1">
                  <span className="font-bold text-zinc-900 text-sm">${order.totalAmount.toFixed(2)}</span>
