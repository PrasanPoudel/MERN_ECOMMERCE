...
export default function ProductDetailPage() {
  ...
  const handleAddToCart = async () => {
    ...
    const res = await dispatch(addToCart({ productId: product._id, quantity: qty }));
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
        ...
        <div>
          <div className="bg-zinc-100 rounded-2xl overflow-hidden aspect-square mb-3">
            <img
              src={product.images?.[activeImg]?.url || 'https://placehold.co/500x500?text=No+Image'}
              alt={product.name}
        ...
        <div className="flex flex-col">
          ...
          <p className="text-3xl font-bold text-zinc-900 mb-4">${product.price.toFixed(2)}</p>
          ...
          <p className="text-zinc-500 text-sm text-justify leading-relaxed mb-6 flex-1">{product.description}</p>
          ...
          {!isOutOfStock && (
            <div className="flex items-center gap-3 mb-5">
              ...
              <div className="flex items-center border border-zinc-200 rounded-xl overflow-hidden">
                ...
                <span className="px-4 py-2 text-sm font-semibold text-zinc-900 min-w-10 text-center">{qty}</span>
                <button
