...
export default function AdminOrderDetail() {
  ...
  if (loading || !order) return <AdminLayout><Spinner size="lg" /></AdminLayout>;
  ...
  return (
    <AdminLayout>
      <div className="flex items-center gap-3 mb-6">
        <Link to="/admin/orders" className="flex items-center gap-1.5 text-sm text-zinc-400 hover:text-zinc-900 transition-colors">
          <RiArrowLeftLine size={16} /> Orders
        </Link>
        <span className="text-zinc-200">/</span>
        <h1 className="text-base font-bold text-zinc-900">#{order._id.slice(-8).toUpperCase()}</h1>
        <span className={`text-xs font-semibold px-2.5 py-1 rounded-xl border ml-1 ${STATUS_STYLES[order.orderStatus] || 'bg-zinc-100 text-zinc-600 border-zinc-200'}`}>
          {order.orderStatus}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-5">
            ...
            <div className="space-y-3">
              {order.items.map((item, i) => (
                <div key={i} className="flex gap-3 items-center">
                  <img src={item.image || 'https://placehold.co/60x60'} alt={item.name} className="w-12 h-12 object-cover rounded-xl bg-zinc-100 shrink-0" />
                  ...
                  <p className="font-bold text-zinc-900 text-sm shrink-0">${(item.price * item.quantity).toFixed(2)}</p>
                </div>
              ))}
            </div>
          ...
          {/* Timeline */}
          <div className="bg-white rounded-2xl border border-zinc-200 p-6">
            <h2 className="text-sm font-bold text-zinc-900 mb-6">Order Timeline</h2>
            <div className="relative pl-4">
              <div className="absolute left-4 top-3 bottom-3 w-px bg-zinc-100" />
              <div className="absolute left-4 top-3 w-px bg-zinc-900 transition-all" style={{ height: `${(currentStepIndex / (ORDER_STEPS.length - 1)) * 100}%` }} />
                  ...
                  return (
                    <div key={step} className="flex items-start gap-4 relative">
                      ...
                      <div className="pt-0.5">
                        ...
                        {histEntry && <p className="text-xs text-zinc-400 mt-0.5">{new Date(histEntry.timestamp).toLocaleString()}</p>}
                      </div>
                    </div>
                  );
                })}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-zinc-200 p-5">
            <h2 className="text-sm font-bold text-zinc-900 mb-3">Update Status</h2>
          ...
          <div className="bg-white rounded-2xl border border-zinc-200 p-5">
            ...
            <p className="font-semibold text-zinc-900 text-sm">{order.user?.name}</p>
