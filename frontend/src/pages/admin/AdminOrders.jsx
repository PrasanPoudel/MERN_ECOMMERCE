...
const STATUS_OPTIONS = ['Pending', 'To Ship', 'Shipped', 'Out for Delivery', 'Delivered'];
const STATUS_STYLES = {
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-zinc-50">
                  {orders.length === 0 ? (
                    <tr><td colSpan={8} className="text-center py-12 text-zinc-400 text-sm">No orders found</td></tr>
                  ) : orders.map((order) => (
                    <tr key={order._id} className="hover:bg-zinc-50 transition-colors">
                      <td className="px-5 py-4 font-mono text-xs font-bold text-zinc-700">#{order._id.slice(-8).toUpperCase()}</td>
                      <td className="px-5 py-4">
                        <p className="font-medium text-zinc-900">{order.user?.name}</p>
                        <p className="text-xs text-zinc-400 mt-0.5">{order.user?.email}</p>
                      </td>
                      <td className="px-5 py-4 text-zinc-500">{order.items.length}</td>
                      <td className="px-5 py-4 font-bold text-zinc-900">${order.totalAmount.toFixed(2)}</td>
                      <td className="px-5 py-4">
                        <span className={`text-xs font-semibold px-2.5 py-1 rounded-xl border ${order.paymentStatus === 'Paid' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-amber-50 text-amber-700 border-amber-100'}`}>
                          {order.paymentStatus}
                        </span>
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4 text-zinc-400 text-xs">{new Date(order.createdAt).toLocaleDateString()}</td>
                      <td className="px-5 py-4">
                        <Link to={`/admin/orders/${order._id}`} className="p-1.5 text-zinc-400 hover:text-zinc-900 hover:bg-zinc-100 rounded-lg transition-colors inline-flex">
